...
describe CommentsMailer do
  describe ".email_topic_owner" do
    context "when a comment is supplied to formulate an email" do
      ...
      let(:mail)    { CommentsMailer.email_topic_owner(comment) }
      ...
      it "has receiver's email in to" do
        mail.to.should include(comment.topic.user.email)
      end

      it "has subject according to topic" do
        mail.subject.should match(comment.topic.subject.first(45))
      end

      it "has topic url in the body" do
        mail.body.should match(topic_url(comment.topic))
      end
    end
  end
end
